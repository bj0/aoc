...
@_coconut_tco
def parse_squad(line, group_type, id, boost):
    n, hp, im, weak, ap, type, init = re.match(r'(\d+).*?(\d+)[^(]*.(?:immune to ([\w, ]+)[^w]*)?(?:weak to ([\w, ]+))?.*? (\d+) (\w+) damage at initiative (\d+)', line).groups()
    n, hp, ap, init = map(int, (n, hp, ap, init))
    im = im if im else ''
    weak = weak if weak else ''
    return _coconut_tail_call(Squad, group_type, id, n, hp, frozenset(im.split(', ')), frozenset(weak.split(', ')), ap + boost, type, init)
...
# target order is (do most dmg to, largest epower, init)
    ...
    for attacker in reversed(sorted(attackers, key=lambda s: s.target_rank)):
        if len(targets) == 0:
            break
        attack = [(attacker.dmg_to(target), *target.target_rank, target) for target in targets]
...
def fighting(fight):
    ...
    for attacker in sorted(combatants, key=lambda s: s.initiative, reverse=True):
        ...
        dmg = attacker.dmg_to(target)
        units = dmg // target.unit_hp
        units = min(units, target.size)
# print(
#     f'{attacker.type} grp {attacker.id} attacks {target.type} grp {target.id} for {dmg} dmg, killing {units} units')
        target.size -= units
# if target.size <= 0:
#     print(f'{target.type} grp {target.id} died!')

# print([u for u in combatants if u.size > 0])


@_coconut_tco
def solve(input, boost=0, p2=False):
    good, bad = parse(input, boost)
...
# print()
    ...
    if not p2:
        ...
        return _coconut_tail_call(sum, (u.size for u in good))
