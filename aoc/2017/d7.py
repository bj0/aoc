...
def part1(input):
    ...
    for line in input.strip().split('\n'):
        towers = [x.strip(' ,') for x in line.split('->')[-1].split()]
        base, weight = (x.strip(' )(') for x in line.split()[:2])
        weights[base] = weight
        if '->' in line:
            map[base] = towers
            held += towers
    ...
    all_towers = [t for i in map.values() for t in i]
    return next(x for x in map.keys() if x not in all_towers), map, weights


def part2(input):
    root, map, weights = part1(input)

    tweights = {}

    def weigh(node):
        if node in tweights:
            return tweights[node]
        w = int(weights[node])
        for tower in map.get(node, []):
            w += int(weigh(tower))
        tweights[node] = w
        return w

    # fill tweights
    print(f"root weight: {weigh(root)}")

    def get_unbalanced_tower(node):
        towers = map[node]
        tw = [tweights[t] for t in towers]
        if tw.count(tw[0]) != len(tw):
            # unbalanced weights!
            if tw.count(tw[0]) == 1:
                return towers[0]
            # not first
            tu = next(x for x in tw if x != tw[0])
            return towers[tw.index(tu)]


    n = root
    last = root
    while n in map:
        t = get_unbalanced_tower(n)
        if t is not None:
            # move up to unbalanced tower
            last = n
            n = t
            continue
        ...
        tu = tweights[n]
        uw = int(weights[n])
        tb = tw[0] if towers[0] != n else tw[1]
        print(last, n, tu, uw, tb)
        return n, tu, tb, uw, uw - (tu - tb)

    print("none unbalanced!")
