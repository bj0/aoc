...
def part1(input):
    ...
    # init regs (so cond variables are present)
    for x in (line.split()[0] for line in lines):
        regs[x] = 0
    print(regs)
    mx = max(regs, key=lambda k: regs[k])
    mxv = regs[mx]
    for line in lines:
        sline = line.split()
        reg, op, n = sline[:3]
        ...
        cond_result = eval(cond, dict(regs))

        if cond_result:
            print(f'{op} reg {reg} by {n} ({regs[reg]} -> {regs[reg]+int(n)})')
            if op == 'inc':
                regs[reg] += int(n)
            elif op == 'dec':
                regs[reg] -= int(n)
            ...
            print('skip!')
    print(f'final regs: {regs}')
    m = max(regs, key=lambda k: regs[k])
    return m, regs[m], mx, mxv
